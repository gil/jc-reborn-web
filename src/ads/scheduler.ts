...
// stopAtPlayScene=false – triggered chunks: don't stop at PLAY_SCENE, continue scanning
//   so IF_IS_RUNNING → FADE_OUT → END can set stopRequested.
function adsPlayChunk(state: AdsState, offset: number, stopAtPlayScene = true): void {
  const bc = state.bc;
  let p = offset;
  ...
  while (continueLoop && p < bc.length) {
    ...
    switch (op) {
      ...
      case ADS.PLAY_SCENE:
        ...
        else if (stopAtPlayScene) continueLoop = false;
        break;
